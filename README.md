...
- 📚 **18 課完整筆記** — 由考試概覽、牌照制度、收費規則、乘客服務、司機行為規範，到全港地方識別、路線規劃、交通標誌分類
- 📝 **互動模擬試** — 352 條選擇題，智能抽題（未答過／曾答錯優先），即時對答案＋詳盡解釋，答題狀態存喺瀏覽器 localStorage（唔會上傳）
- 🗺️ **官方地方及路線題庫** — 255 個地方＋18 條路線（運輸署試題小冊子出處），附 Google Maps 彈窗直接睇位置
- 🚦 **交通標誌圖鑑** — 警告、禁制、指示、資訊標誌官方圖樣內嵌
- 🛑 **發出指令標誌全 104 個** — 由運輸署《道路使用者守則》官方 PDF 抽出，附官方中文名稱；模擬試有「看圖認標誌」分類
- 🔍 **全文搜尋**、🌙 深色模式、📱 手機友善
...
第20課嘅 104 個標誌係由運輸署《道路使用者守則》PDF 自動抽出（需要
`poppler-utils` 嘅 `pdftocairo`／`pdftotext` 同 `Pillow`）：

```bash
python3 tools/extract_signs.py --sheet   # --sheet 另出對照圖供肉眼核對
```

腳本會 cache 官方 PDF 落 `tools/.cache/`，並檢查每個抽出嘅標誌非空白。
