...
.route-map:hover, .quiz-map:hover { background: var(--blue); color: #fff; }
.quiz-map { margin-left: 6px; }
...
(function () {
  ...
  document.addEventListener("click", function (e) {
    ...
    var qbtn = e.target.closest(".quiz-map");
    if (qbtn) {
      e.preventDefault();
      if (qbtn.dataset.from) showRoute(qbtn.dataset.from, qbtn.dataset.to);
      else showPlace(qbtn.dataset.q, qbtn.dataset.label);
    }
  });
  ...
  (function () {
    ...
    function mapBtnHtml(q) {
      if (q.type === "place") {
        return ' <button class="quiz-map" data-q="' + esc(q.mapQ) + '" data-label="' + esc(q.q) + '">🗺️ 地圖</button>';
      }
      if (q.type === "route") {
        return ' <button class="quiz-map" data-from="' + esc(q.from) + '" data-to="' + esc(q.to) + '">🗺️ 路線圖</button>';
      }
      return "";
    }
    ...
    function routeQ(r) { return { type: "route", q: r.s + " → " + r.d, a: r.r, tag: "路線", n: r.n, from: r.s, to: r.d }; }
    function mcQ(m) {
      var ok = null;
    ...
    function answer(btn) {
      ...
      if (picked === q.a) {
        ...
        st.wrong.push({ q: q.q, a: q.a, picked: picked, type: q.type, mapQ: q.mapQ, from: q.from, to: q.to });
        fb.innerHTML = "❌ 錯！正確答案：" + esc(q.a) + mapBtnHtml(q);
      ...
      if (st.wrong.length) {
        qzWrong.innerHTML = "<h4>❌ 錯題重溫（" + st.wrong.length + "題）</h4><ul>" +
          st.wrong.map(function (w) {
            return "<li>" + esc(w.q) + " → 正確：<b>" + esc(w.a) + "</b>（你答：" + esc(w.picked) + "）" + mapBtnHtml(w) + "</li>";
          }).join("") + "</ul>";
      } else {
        qzWrong.innerHTML = "";
      }
